...
\begin{itemize}
  ...
  \item \textbf{Métodos:}
  \begin{itemize}
    \item \texttt{elegir\_pozo(tablero)} : permite seleccionar el pozo desde el cual mover semillas.
  \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Diagrama_secuencia.jpg}
...
El flujo comienza con la creación de una instancia del juego por parte del usuario. En ese momento, se inicializan los componentes internos: el tablero y los dos jugadores. Posteriormente, el usuario invoca el método \texttt{jugar()}, el cual inicia un ciclo repetitivo que continúa hasta que se determina un ganador.

Durante cada turno:
\begin{itemize}
    \item El juego muestra el estado actual del tablero.
    \item El jugador activo elige un pozo desde el cual mover semillas.
    \item Las semillas se distribuyen en los pozos correspondientes, utilizando el método \texttt{mover\_semillas()}.
    \item El sistema verifica si el juego ha terminado usando el método \texttt{verificar\_ganador()}, que a su vez consulta al tablero si alguna fila está vacía.
